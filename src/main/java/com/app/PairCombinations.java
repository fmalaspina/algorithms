...
public class PairCombinations {
    ...
    public static List<List<Pair<Integer, Boolean>>> getAllCombinations(int[][] pairs) {
        List<List<Pair<Integer, Boolean>>> combinations = new ArrayList<>();

        generateCombinations(pairs, 0, new ArrayList<>(), combinations);

        return combinations;
    }

    private static void generateCombinations(int[][] pairs, int pairIndex, List<Pair<Integer, Boolean>> currentCombination, List<List<Pair<Integer, Boolean>>> combinations) {
        if (pairIndex == pairs.length) {
            combinations.add(new ArrayList<>(currentCombination));
            return;
        }

        for (int num : pairs[pairIndex]) {
            currentCombination.add(new Pair<>(num, true)); // Add the number from the left side
            generateCombinations(pairs, pairIndex + 1, currentCombination, combinations);
            currentCombination.remove(currentCombination.size() - 1);

            currentCombination.add(new Pair<>(num, false)); // Add the number from the right side
            generateCombinations(pairs, pairIndex + 1, currentCombination, combinations);
            currentCombination.remove(currentCombination.size() - 1);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        List<List<Pair<Integer, Boolean>>> allCombinations = getAllCombinations(pairs);

        System.out.println("All combinations:");
        for (List<Pair<Integer, Boolean>> combination : allCombinations) {
            for (Pair<Integer, Boolean> pair : combination) {
                System.out.print(pair.getLeft() + (pair.getRight() ? " (left) " : " (right) ") + " ");
            }
            System.out.println();
        }
    }
